...
data SeqBox = forall a. Seqable a => SQB a

class Seqable a where
  doseq :: a -> Parser -> Parser

instance Seqable (Parser -> Parser) where
  doseq f p = f p

instance Seqable Parser where
  doseq a b = PSeq a b

instance Seqable SeqBox where
  doseq (SQB a) b = doseq a b

doseq' :: [SeqBox] -> Parser -> Parser
doseq' [] l = l
doseq' [x] l = doseq x l
doseq' (x:y:xs) l = doseq x $ doseq y $ doseq' xs l
...
array = doseq' [SQB $ pchr '[', SQB $ pmaybe' array'] (pchr ']')
