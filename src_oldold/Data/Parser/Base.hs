...
parse Fail x = error $ "parse Fail "++show x
parse Done x = error $ "parse Done "++show x
...
-- data PT = NP Parser | LAP (PT -> Parser)
...
-- doSeq :: PT -> PT -> PT
-- doSeq (NP a) (NP b) = NP $ Seq a b
-- doSeq (NP a) (LAP b) = LAP $ \n -> Seq a $ b n
-- doSeq (LAP a) b@(NP _) =  NP $ a b
-- doSeq (LAP a) b = LAP $ \n -> a $ doSeq b n
...
-- doAnd :: PT -> PT -> PT
-- doAnd (NP a) (NP b) = NP $ And a b
-- doAnd (NP a) (LAP b) = LAP $ \n -> And a $ b n
-- doAnd (LAP a) (LAP b) = LAP $ \n -> And (a n) (b n)
-- doAnd a b = doAnd b a
